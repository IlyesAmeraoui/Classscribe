...
export default function LoginPage() {
  ...
  const handleContinue = async (e: React.FormEvent) => {
    ...
    if (step === 'email') {
      if (isSignUp) {
        // Pour l'inscription, on passe directement aux détails du compte
      ...
      if (code.length === 6) {
        // Appel API verifyEmail
        const res = await verifyEmail(email, code)
        if (res.success) {
          // Après vérification d'email, connecter automatiquement l'utilisateur
          try {
            const loginRes = await login(email, password)
            if (loginRes.success) {
              window.location.href = '/dashboard'
            } else {
              alert('Email verified but login failed: ' + loginRes.error)
            }
          } catch (error) {
            console.error('Auto-login failed:', error)
            alert('Email verified but auto-login failed')
          }
        } else {
          alert(res.error)
        }
      ...
      try {
        // Appel API register (création du compte)
        const res = await register(email, password, username, profileImage || undefined)
        if (res.success) {
          // Après création du compte, envoyer le code de vérification
          const codeRes = await resendCode(email)
          if (codeRes.success) {
            setIsCodeSent(true)
            setStep('verify-email')
          } else {
            alert('Account created but failed to send verification code: ' + codeRes.error)
          }
        } else {
          alert(res.error)
        }
